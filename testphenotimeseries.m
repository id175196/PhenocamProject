function testphenotimeseries(indir,outdir,maskfiles,masktype,windowsize,sttime,endtime,...
...
l = round(floor(max_doy) - windowsize);

% make matrix to dump smoothed results in
gccsmooth = zeros(floor(max_doy),1);
...
% % prepare maskfile name for output
% maskfile = strrep(maskfile,'.mat','-');
% sep = filesep;
% maskfile = strrep(maskfile,['..' sep],'');
