function phenokmeans(indir,outdir,windowsize,sttime,endtime,...
                        nroi,threshold,numclusters,varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
kmat = zeros(size(mask,1)*size(mask,2),12);
kindex = 1;
...
disp(numclusters);
disp ('hello');
kresults = kmeans(kmat,numclusters);
kresults = reshape(kresults,size(img,2),size(img,1));
kresults = fliplr(kresults);
kresults = rot90(kresults);
figure
imagesc(kresults); colormap(gray)
%}
%{
